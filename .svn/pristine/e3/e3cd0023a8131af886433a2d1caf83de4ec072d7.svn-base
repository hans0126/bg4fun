...
class Auth extends Backend_Controller 
{
	...
	public function editAuth($group_sn)
	{
		...
		$have_auth_list = $this->main_model->GetGroupAuthorityList("sys_admin_group_sn =".$group_sn);
		$data["have_auth_list"] =  $this->main_model->convertArrayToValueArray( $have_auth_list["data"] , "module_sn");        
        
		$data["auth_list"] = $this->main_model->GetTotalModelList();
	...
	{		
		...
		{
			$this->main_model->deleteDB("sys_admin_group_authority", NULL ,array("sys_admin_group_sn"=>$web_admin_group_sn));	
			
			foreach ($auth_ary as $module_sn => $auth_value) 
			{								
				$this->main_model->addData( "sys_admin_group_authority" , array("sys_admin_group_sn"=>$web_admin_group_sn,"module_sn"=>$auth_value));								
			}
		}		
	...
	public function admin()
	{
		...
		$data['url']=array("edit"=>bUrl('editAdmin')
                            ...
                            ,"launch"=>"ajax_admin_launch");
			
		$this->sub_title = $this->lang->line("admin_list");
		
        $table="sys_admin INNER JOIN sys_admin_belong_group ON sys_admin.sn=sys_admin_belong_group.sys_admin_sn
                INNER JOIN sys_admin_group ON sys_admin_group.sn=sys_admin_belong_group.sys_admin_group_sn";
        
        $field="sys_admin.sn
                ,sys_admin.id
                ,sys_admin.start_date
                ,sys_admin.end_date
                ,sys_admin.forever
                ,sys_admin.launch
                ,sys_admin_group.name"; 
        
		$admin_list = $this->main_model->listDB( $table , $field, NULL  , $this->per_page_rows , $this->page , array("sn"=>"asc") );
	...
	public function editAdmin($admin_sn="")
	{
		...
		{
		    $table="sys_admin INNER JOIN sys_admin_belong_group ON sys_admin.sn=sys_admin_belong_group.sys_admin_sn";		    
		    $field="sys_admin.*,sys_admin_belong_group.sys_admin_group_sn as group_sn";		    
			$admin_info = $this->main_model->listDB($table,$field ,"sys_admin.sn =".$admin_sn);
	...
	public function updateAdmin()
	{
        ...
        else 
        {
			...
			if($edit_data["sn"] != FALSE)
			{
				...
				if($arr_return['success'])
				{
					$this->main_model->updateWebAdminGroup($edit_data);
					$this->showSuccessMessage();					
				}
				else 
				{
			...
			else 
			{
				...
				$sys_admin_sn = $this->main_model->addDB( "sys_admin" , $arr_data );          
                
				if($sys_admin_sn > 0)
				{
				
					$edit_data["sn"] = $sys_admin_sn;
					$this->main_model->updateWebAdminGroup($edit_data);
					$this->showSuccessMessage();							
				}
				else 
    ...
    public function ajax_admin_launch() {
      
        $arr_data = array("launch" => $_POST['launch']);
        $arr_return = $this -> main_model -> updateDB("sys_admin", $arr_data, "sn =" . $_POST['sn']);
	...
	public function generateTopMenu()
	{
		...
		{
		    //未完成
			$this->addTopMenu("群組",array("group","editGroup","editAuth","updateGroup"));
		}
	}
}
