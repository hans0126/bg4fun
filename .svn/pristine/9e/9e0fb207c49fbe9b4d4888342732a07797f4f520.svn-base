			...
			<table width="100%" border="0" cellspacing="0" cellpadding="0">
				<tr>
					...
					<td style="text-align:right">
						...
						<option value="<? echo $item["sn"]?>" <?if($item["sn"]==$lan_sn){echo "selected";}?>><? echo $item["language_name"]?></option>
						<?php 
						  endforeach;                        
                        }else{ ?>
                          <option value="<? echo $this->basic_language["sn"]?>"><? echo $this->basic_language["language_name"]?></option>                            
                        <?php }?>								
						</select>
						<select name="module_sn" id="module_sn">
							<? foreach ($module_select_list as $item): ?>
						<option value="<? echo $item["sn"]?>" <?if($item["sn"]==$mod_sn){echo "selected";}?>><? echo $item["title"]?></option>
						<? endforeach; ?>
						</select>
