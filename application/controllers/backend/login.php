		...
		else 
		{
			...
			if(strtolower($edit_data["vcode"]) === strtolower($this->session->userdata('veri_code')))
			{
				...
				if($admin_info["count"] > 0)
				{
					...
					//查詢所屬群組
					//------------------------------------------------------------------------------------------------------------------
					$sys_admin_belong_group = $this->auth_model->listDB("sys_admin_belong_group",NULL,"sys_admin_sn = ".$admin_info["sn"] );
					...
					if($admin_info["is_default"] !== "1")
					{
						...
						$sys_admin_authority_list = $this->auth_model->listDB("sys_admin_group_authority", NULL, $str_conditions );	
