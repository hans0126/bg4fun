...
abstract class Frontend_Controller extends IT_Controller 
{
	...
	function __construct() 
	{
		...
		if(MULTIPLE_LANGUAGES){					
            ...
            //判斷是否有語系檔，並載入
            $lang_path=SYSDIR."/language/".$this->uri->segment(1).".php";
	...
	{
				//判斷是否有語系近來
		$this->language_value = $this->uri->segment(1);
		$this->config->set_item('language_value', $this -> language_value);
		if($this->language_value == FALSE){
			$this->redirectHome();		
		}else{
			...
			if(count($language_list)>0){			
				...
				$this->language_value = $language_list[0]['language_value'];
				$this->language_name = $language_list[0]['language_name'];
			}				
		}
	}
	...
	public function redirectHome()
	{
		...
		//取得預設語系		
			...
			if( sizeof( $list ) > 0 )
			{				
				header("Location:".base_url().$list[0]["language_value"]."/home");
	...
	function display($view, $data = array() ,$controller_name = '', $sub_title = "")
	{
		...
		//$data["has_kv"] = $has_kv;
		$data['content'] = $this->load->view($view, $data, TRUE);
		return $this->load->view('frontend/template_content_view', $data);
	}
